...
@Component({
  selector: 'app-create-cultivator',
  templateUrl: './create-cultivator.component.html',
  styleUrls: ['./create-cultivator.component.scss'],
})
export class CreateCultivatorComponent {
  newProviderCreated!: IProvider;
  createCultivatorForm: UntypedFormGroup = this.formBuilder.group({
  ...
  createProviderObject(action: ModalActions) {
    ...
    try {
      provider = Provider.create(this.getFormData(this.createCultivatorForm));
    } catch (error: any) {
  ...
  createProvider(provider: IProvider, action: ModalActions): void {
    let email!: Email;
    try {
      email = Email.create(this.createCultivatorForm.value.email);
    } catch (error: any) {
      if (error instanceof DomainValidationError) {
        this.errorHandler.handleDomainError(error);
      } else this.errorHandler.handleUnkonwError(error);
    ...
    this.providerService
      ...
      .subscribe({
        next: (res: any) => {
          if (res) {
            this.notifier.showNotification('success', 'Cultivador creado');
  ...
  getFormData(form: UntypedFormGroup): IProvider {
    ...
    return this.formsHelperService.createCultivatorFormData(form);
  }

  get nameControl(): AbstractControl {
    return this.createCultivatorForm.controls['name'];
  }
  get emailControl(): AbstractControl {
    return this.createCultivatorForm.controls['email'];
  }
  get phonePriceControl(): AbstractControl {
    return this.createCultivatorForm.controls['phone'];
  }
  get addressControl(): AbstractControl {
    return this.createCultivatorForm.controls['address'];
  }
}
